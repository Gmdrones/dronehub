(function(){
  ...
  function owner(){return activeOwner||currentUser();}
  function aircraft(){
    if(Array.isArray(activeAircraft))return activeAircraft;
  ...
  function migrateLegacyBatteries(){
    ...
    all.forEach(function(b){
      if(b.userId==='local'&&ownerId!=='local'){b.userId=ownerId;b.aircraftId=b.aircraftId||(a[0]&&a[0].id);changed=true;}
      if(b.userId===ownerId&&a.length&&!a.some(function(x){return String(x.id)===String(b.aircraftId);})){b.aircraftId=a[0].id;changed=true;}
    });
    if(changed){localStorage.setItem('dronehub_baterias',JSON.stringify(all));all.filter(function(b){return b.userId===ownerId;}).forEach(function(b){saveBattery(ownerId,b);});}
  }
  function escapeHtml(value){var e=document.createElement('span');e.textContent=value||'';return e.innerHTML;}
  function render(context){
    if(context&&context.userId)activeOwner=context.userId;
    if(context&&Array.isArray(context.aircraft))activeAircraft=context.aircraft.slice();
    ...
    root.querySelector('#fleetScoreValue').textContent=a.length?Math.round(health)+'%':'—';
    ...
    list.querySelectorAll('[data-delete]').forEach(function(btn){btn.onclick=function(){if(!confirm('Excluir esta bateria?'))return;deleteBattery(owner(),btn.dataset.delete);render();};});
    list.querySelectorAll('[data-edit]').forEach(function(btn){btn.onclick=function(){var b=batteries().find(function(x){return String(x.id)===String(btn.dataset.edit);});if(!b)return;var value=prompt('Quantidade atual de ciclos:',b.cycles||0);if(value===null)return;b.cycles=Math.max(0,Number(value)||0);b.updatedAt=new Date().toISOString();saveBattery(owner(),b);render();};});
  }
