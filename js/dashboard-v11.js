(function(){
  ...
  function mobileMarkup(){
    ...
    var pro=Boolean(window.user&&(user.plan==='pro'||user.role==='admin')),centralHref=pro?'central-voo.html':'precos.html';
    ...
    mobile.innerHTML='<div class="mobile-ops-hero"><div class="mobile-ops-copy"><span class="mobile-ops-eyebrow">DRONE HUB · OPERAÇÃO ATUAL</span><h1>'+esc(greeting()+', '+firstName())+'</h1><div class="mobile-ready"><i></i><span id="mobileReadyText">'+(pro?'Operação pronta para consulta':'Base operacional Free')+'</span></div>'+(pro?'<div class="mobile-weather-line"><div><small>LOCAL</small><strong>'+esc(city)+'</strong></div><div><small>TEMPERATURA</small><strong id="mobileTemp">--</strong></div><div><small>VENTO</small><strong id="mobileWind">--</strong></div></div>':'')+'<a class="mobile-main-cta" href="'+centralHref+'"><span>'+(pro?'Abrir Central de Voo':'Conhecer o Pro')+'</span><b>→</b></a></div></div>'+
      ...
      '<div class="mobile-expand-panel" id="mobileExpandPanel">'+
        ...
        '<nav class="mobile-tools-grid" aria-label="Todas as ferramentas do Drone Hub">'+
          ...
          '<a href="'+centralHref+'"><strong>Central de Voo · Pro</strong><small>Clima e decisão operacional</small></a>'+
  ...
  function syncOperationalSnapshot(detail){var snapshot=detail;try{snapshot=snapshot||JSON.parse(localStorage.getItem('dronehub_operational_snapshot')||'null')}catch(_){}if(!snapshot||!snapshot.weather)return;var c=snapshot.weather.current||{};[['mobileTemp',Math.round(c.temperature_2m)+'°'],['mobileTemp2',Math.round(c.temperature_2m)+'°'],['mobileWind',Math.round(c.wind_speed_10m)+' km/h'],['mobileWind2',Math.round(c.wind_speed_10m)+' km/h'],['mobileGust',Math.round(c.wind_gusts_10m)+' km/h'],['mobileSat',Number(snapshot.weather.space_weather&&snapshot.weather.space_weather.kp||0).toFixed(1)]].forEach(function(x){var el=document.getElementById(x[0]);if(el)el.textContent=x[1];});var status=document.getElementById('mobileStatus'),ready=document.getElementById('mobileReadyText');if(status)status.textContent=snapshot.label;if(ready)ready.textContent=snapshot.label+' · '+snapshot.score+'/100';}
  function init(){updateDesktopKpis();hideEmpty();smartHero();mobileMarkup();toggleMore();syncWeather();syncOperationalSnapshot();var weather=document.getElementById('weatherContent');if(weather)new MutationObserver(syncWeather).observe(weather,{subtree:true,childList:true,characterData:true,attributes:true});window.addEventListener('dronehub:operational-updated',function(e){syncOperationalSnapshot(e.detail);syncWeather();smartHero()});window.addEventListener('dronehub:dashboard-data-updated',function(){updateDesktopKpis();hideEmpty();smartHero()});}
